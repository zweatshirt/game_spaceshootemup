...
public class Enemy_3 : Enemy { // Enemy_3 extends Enemy
    ...
    void Start () {
        ...
        float xMin = -boundCheck.camWidth + boundCheck.radius;
        float xMax = boundCheck.camWidth - boundCheck.radius;
        Vector3 v;

        // Pick a random middle position in the bottom half of the screen
        v = Vector3.zero;
        v.x = Random.Range( xMin, xMax );
        v.y = -boundCheck.camHeight * Random.Range( 2.75f, 2 );
        points[1] = v;

        // Pick a random final position above the top of the screen
        v = Vector3.zero;
        v.y = pos.y;
        v.x = Random.Range( xMin, xMax ); points[2] = v;
